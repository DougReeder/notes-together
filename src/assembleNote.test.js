...
The backup plan was to use mailto:contact@abc.edu?subject=Plan%20Progress&body=good%20stuff`;
  ...
  it("should make each line of a plain text file a paragraph, and append the name", async () => {
  ...
  it("should override the file type of text files if the url field is present", async () => {
    ...
    expect(nodeNote.subtype).toMatch(/^html/);
    expect(nodeNote.nodes.length).toBeGreaterThanOrEqual(4);
  });
  ...
  it("should append the name of an empty file", async () => {
    ...
    const nodeNote = await assembleNote("", "The attached file is empty.", "", [file]);
  ...
  it("should reject if no usable content", async () => {
    const file = new File([], "something dull.rtf", {type: 'text/rtf'});
